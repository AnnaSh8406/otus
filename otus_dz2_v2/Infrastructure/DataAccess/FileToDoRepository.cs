...
using System.Text.Json;
using static otus_dz2_v2.Keyboard;
...
namespace otus_dz2_v2.Infrastructure.DataAccess
{
    internal class FileToDoRepository : IToDoRepository
    {
        ...
        public async Task AddAsync(ToDoItem item, CancellationToken cancellationToken)
        {
            ...
            await JsonSerializer.SerializeAsync(createStream, item, cancellationToken: cancellationToken);

            var fileIndex = await GetFileIndexList(cancellationToken);

            fileIndex.Add(new IndexItem(item.Id, item.User.UserId));

            using var writerIndex = File.Create(_indexFileName);
            await JsonSerializer.SerializeAsync(writerIndex, fileIndex, cancellationToken: cancellationToken);
        }

        public async Task<int> CountActiveAsync(Guid userId, CancellationToken cancellationToken)
        {

            var toDoItemList = await GetAllByUserIdAsync(userId, cancellationToken);
        ...
        public async Task<ToDoItem?> GetAsync(Guid id, CancellationToken cancellationToken)
        {
            ...
            if (fileItem != null)
            {
                ...
                if (File.Exists(fileName))
                {
                    ...
                    toDoItem = await JsonSerializer.DeserializeAsync<ToDoItem>(reader, cancellationToken: cancellationToken);
                }
            }
            return toDoItem;
        }
        ...
        public async Task<IReadOnlyList<ToDoItem>> GetAllByUserIdAsync(Guid userId, CancellationToken cancellationToken)
        {
            ...
            if (Directory.Exists(userDirectory))
            {
                ...
                {
                    using var reader = File.OpenRead(file);
                    var item = await JsonSerializer.DeserializeAsync<ToDoItem>(reader, cancellationToken: cancellationToken);
                    if (item != null)
                    {
        ...
        public async Task<List<IndexItem>> GetFileIndexList(CancellationToken cancellationToken)
        {
            ...
            if (new FileInfo(_indexFileName).Length != 0)
            {
                ...
                fileIndex = await JsonSerializer.DeserializeAsync<List<IndexItem>>(readerIndex, cancellationToken: cancellationToken);
                if (fileIndex == null)
                {
        ...
        public async Task<IReadOnlyList<ToDoItem>> GetByUserIdAndList(Guid userId, Guid? listId, CancellationToken cancellationToken)
        {
            var toDoItemList = await GetAllByUserIdAsync(userId, cancellationToken);
            if (listId == null)
            {
