...
namespace otus_dz2_v2.Infrastructure.DataAccess
{
    ...
    public class FileToDoRepository : IToDoRepository
    {
        ...
        public FileToDoRepository(string baseDirectory)
        {
            ...
            _baseDirect = baseDirectory;

            _indexFile = Path.Combine(_baseDirect, "index.json");
        ...
        private async Task CreateFileIndex()
        {
            ...
            foreach (var folder in Directory.EnumerateDirectories(_baseDirect))
            {
                foreach (var file in Directory.EnumerateFiles(folder, "*.json"))
                {
                    try
                    {
                        ...
                        if (item != null)
                        {
                            indexes.Add(new Index(item.User.UserId, item.Id));
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        private async Task<ToDoItem?> GetByGuid(Guid id, CancellationToken cancellationToken)
        {
            ...
            var filePath = Path.Combine(_baseDirect, indexRecord.UserId.ToString(), $"{id}.json");
            try
            {
        ...
        {
            var userFolder = Path.Combine(_baseDirect, userId.ToString());
        ...
        private async Task Add(ToDoItem item, CancellationToken cancellationToken)
        {
            ...
            var userFolder = Path.Combine(_baseDirect, item.User.UserId.ToString());
            if (!Directory.Exists(userFolder))
            {
            ...
            index.Add(new Index(item.User.UserId, item.Id));
            await WriteFileIndex(index);
        }

        private async Task Update(ToDoItem item, CancellationToken cancellationToken)
        {
            var filePath = Path.Combine(_baseDirect, item.User.UserId.ToString(), $"{item.Id}.json");
            await File.WriteAllTextAsync(filePath, JsonSerializer.Serialize(item, new JsonSerializerOptions { WriteIndented = true }), cancellationToken);
        }
